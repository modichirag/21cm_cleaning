...
def make_rep_plot():
    ...
    def makeplot(bfit, datapp, lss, lww, cc, lbl=None):
        ...
        ax[0].plot(rpfit[0]['k'], rpfit[0]['power']/(rpfit[3]['power']*rpfit[4]['power'])**0.5, ls=lss, lw=lww, color=cc, label=lbl)
        ax[1].plot(rpfit[0]['k'], (rpfit[3]['power']/rpfit[4]['power'])**0.5, ls=lss, lw=lww, color=cc)
    ...
    plt.savefig(figpath + '/rep_L%04d_%0.4f.pdf'%(bs, aa))
